...
test('operator routes enforce isolation, persist drafts/history, block duplicate sends and gate activation',async()=>{
 ...
 globalThis.fetch=async()=>Response.json({id:'report-1'});assert.equal((await post({businessId:'b'},'reports')).status,200);assert.equal((await post({businessId:'b'},'reports')).status,409);
