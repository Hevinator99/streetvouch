...
export function buildWeeklyReport(input:WeeklyReportInput){
  ...
  const breakdownHtml=allComments.length?`<table role="presentation" width="100%" cellspacing="0" cellpadding="0">${categoryOrder.map(category=>`<tr><td style="padding:7px 0"><span style="display:inline-block;width:9px;height:9px;border-radius:9px;background:${categoryColors[category]};margin-right:8px"></span><span style="font-size:13px;color:#425752">${escapeHtml(category)}</span></td><td align="right" style="font-size:14px;font-weight:900;color:#17312f">${categoryCounts.get(category)}</td></tr>`).join("")}</table>`:`<div style="font-size:14px;color:#61706c">Not enough data yet.</div>`;
  ...
  const themeRows=recurring.map(([theme,stat])=>{const dominant=(["positive","negative","mixed","neutral"] as Sentiment[]).sort((a,b)=>stat[b]-stat[a])[0],tone=sentimentStyle[dominant];return `<tr><td style="padding:8px 0;font-size:13px;font-weight:800;color:#34504b">${escapeHtml(theme)}</td><td style="padding:8px 12px"><div style="height:8px;background:#e5ebe7;border-radius:99px;overflow:hidden"><div style="height:8px;width:${Math.max(18,Math.min(100,stat.count*25))}%;background:${tone.color};border-radius:99px"></div></div></td><td align="right" style="padding:8px 0;font-size:11px;color:${tone.color};font-weight:800">${stat.count} · ${dominant}</td></tr>`;}).join("");
