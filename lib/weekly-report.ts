...
export function buildWeeklyReport(input:WeeklyReportInput){
  ...
  const activityChart=`<table role="presentation" width="100%" cellspacing="0" cellpadding="0">${barRow("NFC + QR",interactions,previousInteractions,"#95d84e",chartMax)}${barRow("Page visits",input.current.visits,input.previous.visits,"#367f72",chartMax)}${barRow("Private feedback",input.current.privateMessages,input.previous.privateMessages,"#6c83d8",chartMax)}${barRow("Google reviews",input.current.newGoogleReviews,input.previous.newGoogleReviews,"#efb44e",chartMax)}</table>${journeyHtml}`;
