...
export function buildWeeklyReport(input:WeeklyReportInput){
  ...
  const themeRows=recurring.map(([theme,stat])=>{const dominant=(["positive","negative","mixed","neutral"] as Sentiment[]).sort((a,b)=>stat[b]-stat[a])[0],tone=sentimentStyle[dominant],segments=(["positive","mixed","negative","neutral"] as Sentiment[]).filter(key=>stat[key]).map(key=>`<td width="${percent(stat[key],stat.count)}%" style="height:8px;background:${sentimentStyle[key].color};font-size:0">&nbsp;</td>`).join("");return `<tr><td style="padding:10px 0"><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr><td style="font-size:13px;font-weight:800;color:#34504b">${escapeHtml(theme)}</td><td align="right" style="font-size:11px;color:${tone.color};font-weight:800">${stat.count} mentions · mainly ${dominant}</td></tr><tr><td colspan="2" style="padding-top:6px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-radius:99px;overflow:hidden"><tr>${segments}</tr></table></td></tr></table></td></tr>`;}).join("");
  ...
  const weekBars=(label:string,privateValue:number,reviewValue:number)=>`<tr><td style="padding:10px 0;width:76px;font-size:12px;font-weight:800;color:#52635f">${label}</td><td style="padding:10px 12px"><div style="font-size:10px;color:#61706c;margin-bottom:4px">Private feedback · ${privateValue}</div><div style="height:12px;background:#edf1ef;border-radius:99px;overflow:hidden"><div style="height:12px;width:${percent(privateValue,weeklyMax)}%;background:#7c8fe5;border-radius:99px"></div></div><div style="font-size:10px;color:#61706c;margin:8px 0 4px">Google reviews · ${reviewValue}</div><div style="height:12px;background:#edf1ef;border-radius:99px;overflow:hidden"><div style="height:12px;width:${percent(reviewValue,weeklyMax)}%;background:#2e8b7d;border-radius:99px"></div></div></td></tr>`;
  const activityChart=`<table role="presentation" width="100%" cellspacing="0" cellpadding="0">${weekBars("Last week",input.previous.privateMessages,input.previous.newGoogleReviews)}${weekBars("This week",input.current.privateMessages,input.current.newGoogleReviews)}</table><p style="font-size:10px;line-height:1.45;color:#73807d;margin:8px 0 0">Weekly written feedback received: private messages and confirmed Google reviews are shown separately.</p>`;
