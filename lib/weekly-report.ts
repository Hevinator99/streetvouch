...
export type AttentionFeedback={message:string;status:string;severity:string;contact_requested:number|boolean;contacted_at:string|null;due_at:string|null};
export function feedbackNeedsAttention(item:AttentionFeedback,now:string):boolean{
  if(item.status==="resolved"||item.status==="archived")return false;
  return Boolean((item.contact_requested&&!item.contacted_at)||item.severity!=="normal"||classifyComment(item.message,undefined,Boolean(item.contact_requested)).sentiment==="negative"||(item.due_at&&item.due_at<now));
}
...
export function buildWeeklyReport(input:WeeklyReportInput){
  ...
  const actionCount=input.attentionCount??input.awaitingContact+input.flagged+input.reviewsAwaitingReply;
  ...
  const actionRows=[
    ...
    input.otherAttention?`<tr><td style="padding:11px 0;border-bottom:1px solid #ecd9c2"><b>${input.otherAttention}</b> other private message${input.otherAttention===1?" needs":"s need"} review for negative feedback or an overdue follow-up</td></tr>`:"",
