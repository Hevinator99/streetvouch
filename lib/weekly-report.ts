...
function sentimentBlock(sentiment:NonNullable<WeeklyReportInput["sentiment"]>){
  ...
  const legend=parts.map(part=>`<td width="25%" style="padding:12px 4px 0;text-align:center;vertical-align:top"><div style="font-size:20px;font-weight:800;color:${part.text}">${Math.round(part.value/total*100)}%</div><div style="font-size:11px;color:#61706c;margin-top:3px"><span style="color:${part.color}">●</span> ${part.label}</div></td>`).join("");
  return `<table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr>${segments}</tr><tr>${legend}</tr></table><p style="font-size:11px;line-height:1.5;color:#73807d;margin:13px 0 0">Based on ${plural(total,"written response")} analysed this week. Sentiment is an interpretation and should be read alongside the original comments.</p>`;
}
...
export function buildWeeklyReport(input:WeeklyReportInput){
  ...
  const html=`<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>@media(max-width:560px){.shell{padding-left:18px!important;padding-right:18px!important}.metric{display:block!important;width:100%!important}.metric div{min-height:auto!important}.hero-title{font-size:29px!important}.hide-mobile{display:none!important}}</style></head><body style="margin:0;background:#edf1ee;font-family:Arial,Helvetica,sans-serif;color:#17312f"><div style="display:none;max-height:0;overflow:hidden;opacity:0">${escapeHtml(preheader)}</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#edf1ee"><tr><td align="center" style="padding:24px 10px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:640px;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #dce4df"><tr><td class="shell" style="padding:28px 34px 18px"><table role="presentation" width="100%"><tr><td style="font-size:20px;font-weight:800;letter-spacing:-.4px"><span style="display:inline-block;background:#c7f66b;border-radius:7px;padding:2px 7px;margin-right:6px">✓</span>streetvouch</td><td align="right" style="font-size:12px;color:#687672">${formatDate(input.periodStart)}–${formatDate(input.periodEnd)}</td></tr></table></td></tr><tr><td class="shell" style="padding:24px 34px 30px;background:#17312f;color:#ffffff"><div style="font-size:11px;letter-spacing:.13em;font-weight:800;color:#c7f66b;text-transform:uppercase">${escapeHtml(input.businessName)} · Weekly pulse</div><h1 class="hero-title" style="font-family:Georgia,serif;font-size:36px;line-height:1.12;font-weight:400;margin:12px 0 10px">${escapeHtml(actionHeadline)}.</h1><p style="font-size:15px;line-height:1.6;color:#dce7e3;margin:0">${escapeHtml(summary)}</p></td></tr><tr><td class="shell" style="padding:26px 28px 6px"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;color:#61706c;text-transform:uppercase;margin:0 6px 9px">This week at a glance</div><table role="presentation" width="100%" cellspacing="0" cellpadding="0"><tr>${metricCard("NFC taps and QR scans",String(interactions),trend(interactions,previousInteractions))}${metricCard("Customer page visits",String(input.current.visits),trend(input.current.visits,input.previous.visits))}</tr><tr>${metricCard("New Google reviews",String(input.current.newGoogleReviews),trend(input.current.newGoogleReviews,input.previous.newGoogleReviews))}${metricCard("Private messages",String(input.current.privateMessages),trend(input.current.privateMessages,input.previous.privateMessages))}</tr></table><p style="font-size:11px;line-height:1.5;color:#73807d;margin:8px 7px 0">${input.current.averageRating===null?"No Google rating average is available for this period.":`New Google reviews averaged ${input.current.averageRating.toFixed(1)} out of 5.`} Google opens and confirmed reviews are measured separately; StreetVouch does not claim that every click became a review.</p></td></tr><tr><td class="shell" style="padding:26px 34px 0"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#61706c">Activity</div><h2 style="font-size:22px;margin:7px 0 13px">How customers engaged</h2><div style="background:#f7f9f7;border:1px solid #e1e8e3;border-radius:14px;padding:14px 16px">${activityChart}</div></td></tr><tr><td class="shell" style="padding:26px 34px 0"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#61706c">Sentiment analysis</div><h2 style="font-size:22px;margin:7px 0 13px">How this week felt</h2><div style="border:1px solid #e1e8e3;border-radius:14px;padding:18px">${sentimentBlock(sentiment)}</div></td></tr><tr><td class="shell" style="padding:24px 34px 0"><div style="background:#fff6e8;border:1px solid #f0d9b7;border-radius:14px;padding:19px"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#8a5a24">Do these first</div><table role="presentation" width="100%" style="font-size:14px;line-height:1.5;color:#513d27;margin-top:6px">${actionRows}</table></div></td></tr><tr><td class="shell" style="padding:28px 34px 0"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#61706c">Customer voice</div><h2 style="font-size:22px;margin:7px 0 14px">What customers said</h2>${quoteHtml}<p style="font-size:11px;line-height:1.5;color:#73807d;margin:8px 0 0">Private comments are included without customer contact details. Open the secure owner portal for the full conversation and reply controls.</p></td></tr><tr><td class="shell" style="padding:28px 34px 0"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#61706c">Emerging themes</div><div style="margin-top:11px">${themeHtml}</div></td></tr><tr><td class="shell" style="padding:28px 34px"><div style="background:#edf4e5;border-radius:14px;padding:19px"><div style="font-size:11px;letter-spacing:.12em;font-weight:800;text-transform:uppercase;color:#4f694a">Recommended next step</div><p style="font-size:15px;line-height:1.55;margin:8px 0 0;color:#294333">${escapeHtml(insight)}</p></div><div style="text-align:center;margin-top:26px"><a href="${escapeHtml(url)}" style="display:inline-block;background:#17312f;color:#ffffff;text-decoration:none;padding:14px 22px;border-radius:999px;font-size:14px;font-weight:800">Open customer dashboard →</a></div></td></tr><tr><td style="padding:20px 34px;background:#f7f8f6;border-top:1px solid #e0e7e1;font-size:11px;line-height:1.55;color:#73807d">You receive this operational report because you manage ${escapeHtml(input.businessName)}. It contains a limited summary; customer contact information remains in the secure portal.<br><span style="color:#53635f">Feedback made simple with StreetVouch.</span></td></tr></table></td></tr></table></body></html>`;
