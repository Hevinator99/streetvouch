...
export default async function Visibility({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  if (view === "opportunities") {
    ...
    const [queryResult, pageResult, feedbackResult] = await Promise.all([
  ...
  return <>
    ...
    <main className="sv-dashboard sv-vis">
      ...
      <div className="v-wrap">
        ...
        {view === "opportunities" && <><section className="v-panel"><h2>Searches people use</h2><SearchTable rows={queries} kind="query" /></section><section className="v-panel"><h2>Pages people find</h2><SearchTable rows={pages} kind="page" /></section><section className="v-panel"><h2>Draft actions for your review</h2><ol>{alerts[0] && <li><b>Investigate the traffic change.</b> {alerts[0].action} Evidence: {alerts[0].evidence} Business relevance: protect existing enquiries. Effort: medium.</li>}{queries.find(row => row.impressions >= 20 && row.clicks / row.impressions < .05) && <li><b>Review a frequently seen search.</b> Check the matching page’s title and service details for “{queries.find(row => row.impressions >= 20 && row.clicks / row.impressions < .05)?.value}”. Evidence: many appearances with few clicks. Business relevance: improve qualified visits. Effort: low.</li>}{feedbackTotal >= 5 && <li><b>Use customer themes carefully.</b> Review aggregated feedback themes for useful service-page topics. Evidence: {feedbackTotal} private messages in 90 days. Business relevance: answer real customer questions. Effort: medium. Keep messages private.</li>}</ol>{!alerts.length && !queries.length && <p>Connect Search Console and collect data to receive evidence-backed suggestions.</p>}</section></>}
