...
export { safeEqual } from "../lib/security";
...
export async function createManagerSession(database:D1Database,managerUserId:string,scope:"full"|"password_reset"="full"){const now=new Date(),token=crypto.randomUUID()+crypto.randomUUID().replaceAll("-",""),hash=await hashToken(token),ttl=scope==="full"?7*86400000:15*60*1000,expires=new Date(now.getTime()+ttl).toISOString();await database.prepare("INSERT INTO manager_sessions (token_hash,manager_user_id,expires_at,created_at,last_seen_at,scope) VALUES (?,?,?,?,?,?)").bind(hash,managerUserId,expires,now.toISOString(),now.toISOString(),scope).run();return token;}
...
export async function getPasswordResetSession():Promise<ManagerSession|null>{
  if(!env.DB)return null;const token=(await cookies()).get(MANAGER_COOKIE)?.value;if(!token)return null;const hash=await hashToken(token),now=new Date().toISOString();
  return env.DB.prepare(`SELECT mu.id managerUserId,mu.business_id businessId,mu.email,mu.display_name displayName,mu.role,b.name businessName,b.slug businessSlug FROM manager_sessions ms JOIN manager_users mu ON mu.id=ms.manager_user_id JOIN businesses b ON b.id=mu.business_id WHERE ms.token_hash=? AND ms.expires_at>? AND ms.scope='password_reset' AND mu.active=1`).bind(hash,now).first<ManagerSession>();
}
