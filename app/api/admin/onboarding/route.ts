...
export async function POST(request: Request) {
  ...
  if (body.action === "check") {
    ...
    return json({ ok: true, message: "Checklist updated." });
  }
  ...
  if (body.action === "activate") {
    ...
    return json({ ok: true, message: "Pilot activated." });
  }
  if (body.action === "pause") {
    ...
    return result.meta.changes ? json({ok:true,message:"Pilot paused."}) : json({ok:false,message:"Only an active pilot can be paused."},409);
  }
  if (body.action === "complete") {
    ...
    return result.meta.changes ? json({ok:true,message:"Pilot completed and handed off."}) : json({ok:false,message:"Activate the pilot before completing the handoff."},409);
  }
