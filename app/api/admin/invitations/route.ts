...
export async function POST(request: Request) {
  ...
  const statements = [
    existing ? database.prepare("UPDATE manager_users SET display_name=?,role=?,active=0,invited_at=? WHERE id=?").bind(body.displayName ?? null, body.role ?? "owner", now.toISOString(), managerId) : database.prepare("INSERT INTO manager_users (id,business_id,email,display_name,role,active,invited_at,created_at) VALUES (?,?,?,?,?,0,?,?)").bind(managerId, body.businessId, email, body.displayName ?? null, body.role ?? "owner", now.toISOString(), now.toISOString()),
