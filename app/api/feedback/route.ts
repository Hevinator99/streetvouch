...
export async function POST(request:Request){
  try{
    ...
    const database=db();await ensureBusiness(database);if(!await allow(database,`feedback:${await anonymousKey(request,"feedback")}`,5,3600))return json({ok:false,message:"Too many submissions from this device. Please try again later."},429);
    const severity=seriousTerms.test(message)?"serious":attentionTerms.test(message)?"attention":"normal",id=crypto.randomUUID(),now=new Date().toISOString(),session=typeof body.session==="string"?body.session.slice(0,80):null;
