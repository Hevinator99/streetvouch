...
const redirect=(request:Request,state:string,path="/manager/village-barbers-cobham")=>{const url=new URL(path,request.url);url.searchParams.set("google",state);return Response.redirect(url,303);};
const fail=(request:Request,state:string)=>redirect(request,state);
...
export async function GET(request:Request){
  ...
  if(!manager||!code||!state)return fail(request,"invalid");
  ...
  if(!saved||saved.business_id!==manager.businessId||saved.manager_user_id!==manager.managerUserId)return fail(request,"invalid");
  ...
  try{
    ...
    if(locations.length>1){
      ...
      return Response.redirect(new URL("/manager/village-barbers-cobham/google-location",request.url),303);
    }
    ...
    await syncGoogleReviews(manager.businessId);return redirect(request,"connected");
  }catch(error){console.error("google_callback_failed",error);await database.prepare(`INSERT INTO google_connections (id,business_id,status,last_error,created_at,updated_at) VALUES (?,?,'needs_attention',?,?,?) ON CONFLICT(business_id) DO UPDATE SET status='needs_attention',last_error=excluded.last_error,updated_at=excluded.updated_at`).bind(crypto.randomUUID(),manager.businessId,"Google connection failed",now,now).run();return redirect(request,"failed");}
}
