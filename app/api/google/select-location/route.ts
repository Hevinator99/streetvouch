...
export async function POST(request:Request){
  ...
  const home=manager.businessSlug==="village-barbers-cobham"?`/manager/${manager.businessSlug}`:`/manager/${manager.businessSlug}/onboarding`;if(!connection?.encrypted_refresh_token||!connection.token_iv)return Response.redirect(new URL(`${home}?google=failed`,request.url),303);
  try{
    ...
    await syncGoogleReviews(manager.businessId);await database.prepare("UPDATE onboarding_checks SET google_connection_tested=1,updated_at=? WHERE business_id=?").bind(new Date().toISOString(),manager.businessId).run();return Response.redirect(new URL(`${home}?google=connected`,request.url),303);
  }catch(error){console.error("google_location_selection_failed",error);await database.prepare("UPDATE google_connections SET status='sync_failed',last_error=?,updated_at=? WHERE business_id=?").bind("Location selection or review sync failed",new Date().toISOString(),manager.businessId).run();return Response.redirect(new URL(`${home}?google=failed`,request.url),303);}
}
