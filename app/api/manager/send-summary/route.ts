...
function analysisSummary(rows:Row[]){const tally=new Map<string,number>(),sentiment={positive:0,mixed:0,negative:0,neutral:0};for(const row of rows){if(row.sentiment in sentiment)sentiment[row.sentiment as keyof typeof sentiment]++;try{for(const theme of JSON.parse(row.themes) as string[])tally.set(theme,(tally.get(theme)??0)+1);}catch{}}return{themes:[...tally.entries()].sort((a,b)=>b[1]-a[1]).map(([theme])=>theme),sentiment};}
...
export async function POST(request:Request){
  ...
  const [currentEvents,previousEvents,currentFeedback,previousFeedback,currentReviews,previousReviews,outstanding,analysisRows]=await Promise.all([
    ...
    database.prepare("SELECT sentiment,themes FROM ai_analyses WHERE business_id=? AND created_at>=? ORDER BY created_at DESC LIMIT 100").bind(session.businessId,periodStart).all<Row>(),
  ]);
