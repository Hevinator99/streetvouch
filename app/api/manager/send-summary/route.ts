...
export async function POST(request:Request){
  ...
  const [currentEvents,previousEvents,currentFeedback,previousFeedback,currentReviews,previousReviews,outstanding,analysisRows]=await Promise.all([
    ...
    database.prepare("SELECT id,message,severity,contact_requested FROM feedback WHERE business_id=? AND created_at>=? ORDER BY created_at DESC LIMIT 100").bind(session.businessId,periodStart).all<Row>(),
    ...
    database.prepare("SELECT id,reviewer_name,rating,comment FROM google_reviews WHERE business_id=? AND google_created_at>=? ORDER BY google_created_at DESC LIMIT 100").bind(session.businessId,periodStart).all<Row>(),
