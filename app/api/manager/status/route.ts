...
export async function POST(request:Request){
  ...
  const owned=await database.prepare("SELECT id FROM feedback WHERE id=? AND business_id=?").bind(body.id,session.businessId).first();
  if(!owned)return json({ok:false,message:"Feedback not found."},404);
